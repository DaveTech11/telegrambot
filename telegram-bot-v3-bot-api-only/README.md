...
## V3 Telegram Bot API only
This build does not use GramJS, MTProto, API_ID, or API_HASH. It uses the Telegram Bot API through Telegraf and requires only BOT_TOKEN plus OWNER_IDS/TARGET_CHATS in config.js.

The bot profile-picture update feature was removed because the standard Bot API does not provide a method for a bot to change its own profile photo.
